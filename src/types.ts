...
export interface AnimalConfig {
   ...
   * 頭の大きさの倍率（既定 1）。
   *
   * 頭の半径は体高から決まる（けもの h*0.26 / とり h*0.24 / 首長 h*0.11）。
   * 1歳半は顔を見るので既定では大きめに取っているが、**参照画像の頭が
   * 小さい動物では、それが輪郭の最大の食い違いになる**。
   * 実測（M1・2026-09-13）: きりんは頭の幅が体高の 0.221 あり、
   * 参照の 0.107 の2倍だった。プテラノドンも同じ。
   * `bodyWidth` を絞っても頭は縮まない（頭は体高から決まるため）ので、
   * ここで別に絞る。
   */
  headScale?: number;
