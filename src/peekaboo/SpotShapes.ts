...
const PALETTES: Record<SpotKind, Palette> = {
  ...
  // さらに黄土（オーカー）寄りに振った（2026-09-13）。**明度は動かさない**。
  // サイクル24 で素の色を暗くしたら「じゃがいも」に見えたので、
  // L\* を 56.1/69.9/39.8 に固定したまま、色相を 83° → 74° と橙へ寄せて
  // 彩度 C\* だけを 27/30/21 → 33/36/27 に上げてある。
  // 岩の仲間は5場面に出るが、**5箇所すべてで背景画との彩度差が縮む**側なので
  // 引き換えが無い（うみ・のうじょう・どうぶつえん・きょうりゅう は既に基準内）
  rock: { body: 0xa67f4f, cover: 0xcca46b, accent: 0x775834 },
 ...
 * ==========================================================================
 */
function fitBackPlates(shape: SpotShape, kind: SpotKind): void {
  ...
  // **動物の幅を下限にする。**
  // 見えている本体だけに合わせると、本体を細くしたときに背板まで縮んで、
  // **隠れている動物が背板の左右からはみ出す**。
  // 岩の庇を板から塊に替えて横幅を詰めたとき、うみ の いわ で
  // 15×15 の格子のうち 8点から体が覗いた（2026-09-08 の実測）。
  // 背板の役目は「後ろから透けないように塞ぐ」ことなので、
  // 隠れる動物がいちばん広いときの幅（`mouthWidth`）を下回ってはいけない
  const wantW = Math.max(_backBox.max.x - _backBox.min.x, shape.mouthWidth * 1.35);
  ...
  for (const back of backs) {
    // **背板の形は、前面の形に合わせる。**
    //
    // 丸い塊（岩・鉢・くさむら・たまご）に四角い背板を当てると、
    // まわりに板の角が残る（実機の絵で灰色／茶色の長方形が見えていた）。
    // 逆に、四角い隠れ場所（はこ・カーテン・ふとん・とびら）に楕円を当てると、
    // **前面の四角からはみ出した弧が黒い三日月として覗く**
    // （判定の実測: かーてんの左に暗いえんじ色の三日月、ふとんの上端に黒い弧）。
...
function buildSpotShape(kind: SpotKind, spotY: number, spotX: number, seed: number): SpotShape {
  ...
  switch (kind) {
    ...
    case 'rock':
      return createRock(palette, spotY, seed);
    case 'water':
 ...
 */
function createRock(p: Palette, spotY: number, seed: number): SpotShape {
  const rng = seededRandom(seed);
  const group = new THREE.Group();
  ...
  for (const side of [-1, 1]) {
    ...
    //
    // **塞ぐための板を置かない**（2026-09-08）。
    // 球3個だと上下の球のあいだに菱形の隙間が残るので、以前は平らな板を
    // 1枚重ねて塞いでいた。ところが球より奥にあるその板が、**空を背に
    // 四角い面としてはっきり見え**、判定で5場面すべてに減点された。
    // 角丸にしても、楕円にしても、大きさを詰めても消えなかった
    // （詰めると今度は遮蔽が漏れる）。
    //
    // なので**球の数で塞ぐ**。3個 → 6個にして、隙間の位置に重ねる。
    // 塞ぐものが球なら、はみ出しても岩に見える。
    // 6個を1つのジオメトリに結合するので draw call は増えない。
    // ==================================================================
    const blobs: [number, number, number, number, number][] = [
      [0, -H * 0.5, 0, 0.5, 0.62],
      [side * 0.1, -H * 0.06, 0.02, 0.46, 0.5],
      [side * -0.08, H * 0.28, -0.02, 0.4, 0.42],
      // ここから下が「隙間を塞ぐ球」。**外側にも要る**
      // （外側が足りないと、うみ の いわ の右の縁で 3点漏れた）
      [side * 0.34, -H * 0.28, -0.01, 0.46, 0.5],
      [side * -0.28, -H * 0.22, -0.01, 0.46, 0.5],
      [0, -H * 0.85, -0.01, 0.48, 0.44],
    ...
    const chunks = new THREE.Mesh(mergeGeometries(parts), stone);
    chunks.name = `rock.chunks.${side < 0 ? 'l' : 'r'}`;
    half.add(chunks);
