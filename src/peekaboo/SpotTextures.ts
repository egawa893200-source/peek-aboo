...
const stone: Painter = (ctx, rng) => {
  ...
  for (let i = 0; i < 46; i++) {
    ...
    blob(
      ...
      5 + rng() * 14,
      4 + rng() * 12,
      // **無彩色の斑点を置かない**（2026-09-13）。黒と白を重ねると平均の彩度が
      // 灰へ引っ張られる。岩の V4（背景画との彩度差）は残り 2.7 まで詰まっているのに、
      // 素の色をこれ以上濃くすると「じゃがいも」に見えた（サイクル24の実測）。
      // 斑点のほうを土の色・日に焼けた砂の色にすると、素の色を変えずに平均の彩度が上がる
      dark
        ? `rgba(58,38,16,${(0.06 + rng() * 0.14).toFixed(3)})`
        : `rgba(255,240,198,${(0.06 + rng() * 0.16).toFixed(3)})`
    );
  }
  for (let i = 0; i < 220; i++) {
    blob(ctx, rng() * SIZE, rng() * SIZE, 1 + rng() * 2, 1 + rng() * 2, `rgba(58,38,16,${(0.06 + rng() * 0.12).toFixed(3)})`);
  }
  for (let i = 0; i < 3; i++) {
